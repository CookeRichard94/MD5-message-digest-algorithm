...
void MD5Update (MD5_CTX *ctx, unsigned char *input, unsigned int inputLen)
{
  ...
  if ((ctx->count[0] += ((UINT4)inputLen << 3)) < ((UINT4)inputLen << 3))
  ctx->count[1]++;
  ...
  if (inputLen >= partLen) {
    MD5_memcpy((POINTER)&ctx->buffer[index], (POINTER)input, partLen);
  ...
  MD5_memcpy((POINTER)&ctx->buffer[index], (POINTER)&input[i], inputLen-i);
}
...
void MD5Final (unsigned char digest[16], MD5_CTX *ctx)
{
  ...
  // Pad out so that the result will be 56 mod 64
  index = (unsigned int)((ctx->count[0] >> 3) & 0x3f);
